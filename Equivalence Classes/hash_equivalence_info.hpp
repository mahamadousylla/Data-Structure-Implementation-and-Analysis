...
//    HashSet<T> histogram;
    ics::ArrayMap<int, int> histogram;
    double maximum = 0;
    double total_size = 0;
    //Helper methods
...
T HashEquivalence<T,thash>::compress_to_root (T a) {
  ...
  auto current_size = compress_set.size();
  total_size += current_size;
  if (current_size > maximum) {
     maximum = current_size;
  }
  histogram[current_size]++;
  compress_set.clear();
  return to_root;
}
...
std::string HashEquivalence<T,thash>::equivalence_info () const {
  std::ostringstream answer;
  int total = 0;
  answer << "Max size of compress_set:           " << maximum     << std::endl << std::endl;
  for (int i = 0; i < histogram.size(); i++) {
     total += histogram[i];
     answer << "compress_set size is: " << i << "            " << " compress_to_root was called " << histogram[i] << " times(s)" << std::endl;
  }
  auto av = total_size/total;
  answer << "Histogram:                          " << histogram << std::endl;
  answer << std::endl << "compress_root called:               " << total << " times" << std::endl << std::endl;
  answer << "Average size of compress_set:       " << av << std::endl;
