...
typedef ics::HashSet<std::string, hash_string>           FollowSet;
...
int helperForHash(const WordQueue &queue) {
...
//  print_corpus (sort not print) time = 0.093

//Only ArrayQueue Implementations (HeapPriorityQueue, HashMap, and HashSet)
//  read_corpus time                   = 0.436
//  print_corpus (sort not print) time = 0.25

//Briefly explain (in a comment below) what is interesting
//about the times above (and explain why it occurs).

// Its intresting that the more efficient data structure is the ArraySet over the HashSet.
// It appears this has to do with all the time that is spent resizing the
// HashSet. During the re-hashing, all values are reshashed for each bin and
// we can see how this is more costly in terms of time. (which includes adding trailer nodes with no values in every bin)